...
#ifndef __QN_INSTANCE_H__
#define __QN_INSTANCE_H__
...
class QN_BLEInstance : public BLEInstanceBase
{
public:
    QN_BLEInstance(void);
    virtual ~QN_BLEInstance(void);
    ...
    virtual GattServer &getGattServer() {
        if (gattServerInstance == NULL) {
            gattServerInstance = new QN_GattServer();
        }
...
//    }
     ...
     */
    virtual const QN_Gap &getGap() const  {
        return gapInstance;
    };
     ...
     *        internal pointer has been declared mutable.
     */
    virtual const QN_GattServer &getGattServer() const {
        if (gattServerInstance == NULL) {
            gattServerInstance = new QN_GattServer();
        }
        return *gattServerInstance;
...
public:
    static QN_BLEInstance& Instance(BLE::InstanceID_t instanceId);
...
private:
    QN_Gap gapInstance; /**< Gap instance whose reference is returned from a call to
                                   * getGap(). Unlike the GattClient, GattServer and
                                   * SecurityManager, Gap is always needed in a BLE application. */

private:
    mutable QN_GattServer      *gattServerInstance;      /**< Pointer to the GattServer object instance.
