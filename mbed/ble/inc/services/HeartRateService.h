...
* @brief BLE Service for HeartRate. This BLE Service contains the location of the sensor and the heart rate in beats per
* minute.
...
class HeartRateService
{
   ...
   public:
    ...
    HeartRateService(BLE &_ble, uint8_t hrmCounter, uint8_t location)
        : ble(_ble),
          ...
          hrmLocation(GattCharacteristic::UUID_BODY_SENSOR_LOCATION_CHAR, &hrLocation),
   ...
   protected:
    ...
    {
        hrmRate.requireSecurity(SecurityManager::SECURITY_MODE_ENCRYPTION_WITH_MITM);

        GattCharacteristic *charTable[] = {&hrmRate, &hrmLocation, &controlPoint};
        GattService hrmService(GattService::UUID_HEART_RATE_SERVICE, charTable,
                               sizeof(charTable) / sizeof(GattCharacteristic *));

        ble.gattServer().addService(hrmService);
        ble.gattServer().onDataWritten(this, &HeartRateService::onDataWritten);
    }
