...
enum hci_opcode
{
    ...
    HCI_DBG_USER_EXT_FUNC_CMD_OPCODE = 0xFFF0,
    HCI_DBG_READ_CHIP_ID_CMD_OPCODE = 0xFFF1,
    HCI_DBG_MODEM_TX_TEST_CMD_OPCODE = 0xFFF2,
    HCI_DBG_MODEM_RX_TEST_CMD_OPCODE = 0xFFF3,
    HCI_DBG_MODEM_END_TEST_CMD_OPCODE = 0xFFF4,
    HCI_DBG_SET_HFXO_LOAD_CAP_CMD_OPCODE = 0xFFF5,
    HCI_DBG_SAVE_HFXO_LOAD_CAP_CMD_OPCODE = 0xFFF6,
    HCI_DBG_SET_LFXO_LOAD_CAP_CMD_OPCODE = 0xFFF7,
    HCI_DBG_SAVE_LFXO_LOAD_CAP_CMD_OPCODE = 0xFFF8,
};
...
struct hci_le_set_phy_cmd
{
    ...
    /// Preferred PHYS for RX
    uint8_t rx_phys;
    /// PHYS option for LE coded PHY
    uint16_t phys_option;
};
...
/// Common structure for Command Complete Event of HCI Debug Read Memory/Flash/Param complete event parameters - vendor
/// specific
...
/// HCI Debug user extended function complete event parameters - vendor specific
struct hci_user_ext_func_cmd_cmp_evt
{
    /// status
    uint8_t status;
    /// operateion code
...
/// HCI Debug read chip id complete event - vendor specific
struct hci_vs_read_chip_id_cmd_cmp_evt
{
    /// status
    uint8_t status;
    /// chip id
...
/// HCI Debug modem tx test - vendor specific
struct hci_vs_modem_tx_test_cmd
{
    /// Frequency
    uint8_t tx_freq;
    /// Test mode: modulation
    uint8_t mode;
    /// Payload type
    uint8_t payload_type;
};

/// HCI Debug modem rx test - vendor specific
struct hci_vs_modem_rx_test_cmd
{
    /// Frequency
    uint8_t rx_freq;
};

/// HCI Debug set high frequency XTAL load cap - vendor specific
struct hci_vs_set_hfxo_loadcap_cmd
{
    /// Load cap
    uint32_t loadcap;
};

/// HCI Debug set low frequency XTAL load cap - vendor specific
struct hci_vs_set_lfxo_loadcap_cmd
{
    /// Load cap
    uint32_t loadcap;
};
