...
__FORCEINLINE void *ke_malloc(uint32_t size)
{
    return kmalloc(size, GFP_KERNEL);
}

__FORCEINLINE void ke_free (void * mem_ptr)
{
    kfree(mem_ptr);
}
...
__FORCEINLINE void *ke_malloc(uint32_t size) { return malloc(size); }

__FORCEINLINE void ke_free(void * mem_ptr) { free(mem_ptr); }
