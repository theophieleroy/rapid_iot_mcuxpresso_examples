...
#define CMD(opcode, dest_ll, dest_hl, pkupk, par_size_max, par_fmt, ret_fmt)                                    \
    {                                                                                                           \
        HCI_##opcode##_CMD_OPCODE, (dest_ll << HCI_CMD_DEST_LL_POS) | (dest_hl << HCI_CMD_DEST_HL_POS) | pkupk, \
            par_size_max, (void *)par_fmt, (void *)ret_fmt                                                      \
    }
/// Macro for building an event descriptor in split mode (with parameters packing/unpacking)
#define EVT(code, dest_hl, pkupk, par_fmt)                                              \
    {                                                                                   \
        HCI_##code##_EVT_CODE, (dest_hl << HCI_EVT_DEST_HL_POS), pkupk, (void *)par_fmt \
    }
/// Macro for building an event descriptor in split mode (with parameters packing/unpacking)
#define LE_EVT(subcode, dest_hl, pkupk, par_fmt)                                              \
    {                                                                                         \
        HCI_##subcode##_EVT_SUBCODE, (dest_hl << HCI_EVT_DEST_HL_POS), pkupk, (void *)par_fmt \
    }
/// Macro for building an event descriptor in split mode (with parameters packing/unpacking)
#define DBG_EVT(subcode, dest_hl, pkupk, par_fmt)                                             \
    {                                                                                         \
        HCI_##subcode##_EVT_SUBCODE, (dest_hl << HCI_EVT_DEST_HL_POS), pkupk, (void *)par_fmt \
    }
...
#define CMD(opcode, dest_ll, dest_hl, pkupk, par_size_max, par_fmt, ret_fmt)                           \
    {                                                                                                  \
        HCI_##opcode##_CMD_OPCODE, (dest_ll << HCI_CMD_DEST_LL_POS) | (dest_hl << HCI_CMD_DEST_HL_POS) \
    }
/// Macro for building an event descriptor in full mode (without parameters packing/unpacking)
#define EVT(code, dest_hl, pkupk, par_fmt)                      \
    {                                                           \
        HCI_##code##_EVT_CODE, (dest_hl << HCI_EVT_DEST_HL_POS) \
    }
/// Macro for building an event descriptor in full mode (without parameters packing/unpacking)
#define LE_EVT(subcode, dest_hl, pkupk, par_fmt)                      \
    {                                                                 \
        HCI_##subcode##_EVT_SUBCODE, (dest_hl << HCI_EVT_DEST_HL_POS) \
    }
/// Macro for building an event descriptor in full mode (without parameters packing/unpacking)
#define DBG_EVT(subcode, dest_hl, pkupk, par_fmt)                     \
    {                                                                 \
...
struct hci_env_tag
{
    ...
    struct hci_ble_acl_con_tag ble_acl_con_tab[FW_BLE_CONNECTION_MAX];
 ...
 *****************************************************************************************
 */
// enum HCI_PACK_STATUS hci_util_pack(uint8_t* inout, uint16_t* inout_len, const char* format); // for IAR
 ...
 *****************************************************************************************
 */
// enum HCI_PACK_STATUS hci_util_unpack(uint8_t* out, uint8_t* in, uint16_t* out_len, uint16_t in_len, const char*
// format); // for IAR
