...
#define EA_CLOCK_CORR_LAT (ble_config.app->ea_clock_corr_lat)

/// Set ASAP settings
#define EA_ASAP_STG_SET(evt, type, parity, to_protect, resched_att, prio_inc)                                         \
    evt->asap_settings = ((((type) << 14) & 0xC000) | (((parity) << 12) & 0x3000) | (((to_protect) << 11) & 0x0800) | \
...
#define EA_ASAP_STG_TYPE_SET(evt, type) \
    (evt->asap_settings = ((evt->asap_settings & ~(0xC000)) | (((type) << 14) & 0xC000)))
...
#define EA_ASAP_STG_RESCHED_ATT_SET(evt, resched_att) \
    (evt->asap_settings = ((evt->asap_settings & ~(0x03F0)) | (((resched_att) << 4) & 0x03F0)))
