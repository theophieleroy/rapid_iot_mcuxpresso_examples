                                         ...
                                         ke_task_id_t const src_id)
{
    ...
    if (ke_state_get(dest_id) == CPPS_IDLE)
    {
        ...
        struct cpps_get_adv_data_rsp *rsp =
            KE_MSG_ALLOC_DYN(CPPS_GET_ADV_DATA_RSP, src_id, dest_id, cpps_get_adv_data_rsp, CPP_CP_MEAS_ADV_MAX_LEN);
        // Set status
            ...
            if (CPPS_IS_FEATURE_SUPPORTED(cpps_env->features, CPP_FEAT_WHEEL_REV_DATA_SUPP))
            {
                ...
                if (param->parameters.cumul_wheel_rev < 0)
                {
                    cpps_env->cumul_wheel_rev =
                        (co_abs(param->parameters.cumul_wheel_rev) > cpps_env->cumul_wheel_rev) ?
                            0 :
    ...
    if (ke_state_get(dest_id) == CPPS_IDLE)
    {
        ...
        else
        {
            ...
            struct cpps_ntf_cp_vector_rsp *rsp =
                KE_MSG_ALLOC(CPPS_NTF_CP_VECTOR_RSP, src_id, dest_id, cpps_ntf_cp_vector_rsp);
                                       ...
                                       ke_task_id_t const src_id)
{
    ...
    if (cpps_env != NULL)
    {
        ...
        if (param->handle != (CPPS_HANDLE(CPS_IDX_VECTOR_NTF_CFG)))
        {
            ...
            if ((param->handle == (CPPS_HANDLE(CPS_IDX_CTNL_PT_VAL))) && (ctl_pt_status != CPP_CTNL_PT_RESP_SUCCESS))
            {
                                          ...
                                          ke_task_id_t const src_id)
{
    if (ke_state_get(dest_id) == CPPS_IDLE)
    {
        ...
        if ((att_idx == CPS_IDX_CP_MEAS_NTF_CFG) || (att_idx == CPS_IDX_CP_MEAS_BCST_CFG) ||
            (att_idx == CPS_IDX_VECTOR_NTF_CFG) || (att_idx == CPS_IDX_CTNL_PT_IND_CFG))
        {
                                       ...
                                       ke_task_id_t const src_id)
{
    ...
    if (ke_state_get(dest_id) == CPPS_IDLE)
    {
        ...
        struct gattc_write_cfm *cfm =
            KE_MSG_ALLOC(GATTC_WRITE_CFM, KE_BUILD_ID(TASK_GATTC, param->conidx), dest_id, gattc_write_cfm);
                                    ...
                                    ke_task_id_t const src_id)
{
    ...
    if (ke_state_get(dest_id) == CPPS_BUSY)
    {
        ...
        if (status != GAP_ERR_NO_ERROR)
        {
            ...
            cpps_send_cmp_evt(param->conidx, prf_src_task_get(&cpps_env->prf_env, param->conidx),
                              prf_dst_task_get(&cpps_env->prf_env, param->conidx), cpps_env->operation, status);
        }
    }
        ...
        switch (param->operation)
        {
            ...
            case (GATTC_INDICATE):
            {
                ...
                cpps_send_cmp_evt(conidx, prf_src_task_get(&cpps_env->prf_env, conidx),
                                  prf_dst_task_get(&cpps_env->prf_env, conidx), cpps_env->operation, param->status);
