 ...
 ****************************************************************************************
 */
static uint8_t cppc_init(
    struct prf_task_env *env, uint16_t *start_hdl, uint16_t app_task, uint8_t sec_lvl, void *params)
{
    uint8_t idx;
    //-------------------- allocate memory required for the profile  ---------------------

    struct cppc_env_tag *cppc_env = (struct cppc_env_tag *)ke_malloc(sizeof(struct cppc_env_tag), KE_MEM_ATT_DB);

    // allocate CPPC required environment variable
    env->env = (prf_env_t *)cppc_env;

    cppc_env->prf_env.app_task = app_task | (PERM_GET(sec_lvl, SVC_MI) ? PERM(PRF_MI, ENABLE) : PERM(PRF_MI, DISABLE));
    cppc_env->prf_env.prf_task = env->task | PERM(PRF_MI, ENABLE);

    // initialize environment variable
    env->id = TASK_ID_CPPC;
    env->desc.idx_max = CPPC_IDX_MAX;
    env->desc.state = cppc_env->state;
    env->desc.default_handler = &cppc_default_handler;
...
const struct prf_task_cbs cppc_itf = {
    cppc_init, cppc_destroy, cppc_create, cppc_cleanup,
};
...
void cppc_enable_rsp_send(struct cppc_env_tag *cppc_env, uint8_t conidx, uint8_t status)
{
    ...
    struct cppc_enable_rsp *rsp = KE_MSG_ALLOC(CPPC_ENABLE_RSP, prf_dst_task_get(&(cppc_env->prf_env), conidx),
...
{
    // Send the message
    struct cppc_cmp_evt *evt = KE_MSG_ALLOC(CPPC_CMP_EVT, dest_id, src_id, cppc_cmp_evt);
...
void cppc_send_cmp_evt(struct cppc_env_tag *cppc_env, uint8_t conidx, uint8_t operation, uint8_t status)
{
    ...
    // Send the message
    struct cppc_cmp_evt *evt = KE_MSG_ALLOC(CPPC_CMP_EVT, prf_dst_task_get(&(cppc_env->prf_env), conidx),
...
uint16_t cppc_get_read_handle_req(struct cppc_env_tag *cppc_env, uint8_t conidx, struct cppc_read_cmd *param)
{
    // Attribute Handle
                                       ...
                                       uint16_t *handle)
{
    ...
    switch (param->desc_code)
    {
        ...
        case (CPPC_RD_WR_CTNL_PT_CFG):
        {
            if ((param->ntfind_cfg == PRF_CLI_STOP_NTFIND) || (param->ntfind_cfg == PRF_CLI_START_IND))
            {
                *handle = cppc_env->env[conidx]->cps.descs[CPPC_DESC_CTNL_PT_CL_CFG].desc_hdl;
...
uint8_t cppc_unpack_meas_ind(uint8_t conidx, struct gattc_event_ind const *param, struct cppc_env_tag *cppc_env)
{
    ...
    struct cppc_value_meas_ind *ind = KE_MSG_ALLOC(CPPC_VALUE_IND, prf_dst_task_get(&(cppc_env->prf_env), conidx),
...
{
    // CP Measurement value has been received
    struct cppc_value_ind *ind =
        KE_MSG_ALLOC_DYN(CPPC_VALUE_IND, prf_dst_task_get(&cppc_env->prf_env, conidx),
                         prf_src_task_get(&cppc_env->prf_env, conidx), cppc_value_ind, param->length);
...
uint8_t cppc_unpack_ctln_pt_ind(uint8_t conidx, struct gattc_event_ind const *param, struct cppc_env_tag *cppc_env)
{
    ...
    struct cppc_ctnl_pt_rsp *ind = KE_MSG_ALLOC(CPPC_CTNL_PT_RSP, prf_dst_task_get(&cppc_env->prf_env, conidx),
    ...
    if ((ind->rsp.resp_value == CPP_CTNL_PT_RESP_SUCCESS) && (param->length >= 3))
    {
        switch (ind->rsp.req_op_code)
        {
            ...
            {
                offset +=
                    prf_unpack_date_time((uint8_t *)&(param->value[offset]), &(ind->rsp.value.factory_calibration));
            }
            break;
