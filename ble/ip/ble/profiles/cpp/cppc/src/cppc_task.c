...
const struct prf_char_def cppc_cps_char[CPP_CPS_CHAR_MAX] = {
        ...
        /// Sensor Location
        [CPP_CPS_SENSOR_LOC_CHAR] = {ATT_CHAR_SENSOR_LOC, ATT_MANDATORY, ATT_CHAR_PROP_RD},
        /// Vector
        [CPP_CPS_VECTOR_CHAR] = {ATT_CHAR_CP_VECTOR, ATT_OPTIONAL, ATT_CHAR_PROP_NTF},
        /// SC Control Point
        [CPP_CPS_CTNL_PT_CHAR] = {ATT_CHAR_CP_CNTL_PT, ATT_OPTIONAL, ATT_CHAR_PROP_WR | ATT_CHAR_PROP_IND},
};
...
const struct prf_char_desc_def cppc_cps_char_desc[CPPC_DESC_MAX] = {
        ...
        [CPPC_DESC_CP_MEAS_CL_CFG] = {ATT_DESC_CLIENT_CHAR_CFG, ATT_MANDATORY, CPP_CPS_MEAS_CHAR},

        /// CP Measurement Char. - Server Characteristic Configuration
        [CPPC_DESC_CP_MEAS_SV_CFG] = {ATT_DESC_SERVER_CHAR_CFG, ATT_OPTIONAL, CPP_CPS_MEAS_CHAR},
        ...
        [CPPC_DESC_CTNL_PT_CL_CFG] = {ATT_DESC_CLIENT_CHAR_CFG, ATT_OPTIONAL, CPP_CPS_CTNL_PT_CHAR},
};
    ...
    if (state == CPPC_IDLE)
    {
        ...
        else
        {
            ...
            if (hdl != ATT_INVALID_SEARCH_HANDLE)
            {
                ...
                prf_read_char_send(&(cppc_env->prf_env), conidx, cppc_env->env[conidx]->cps.svc.shdl,
        ...
        do
        {
            ...
            if (cppc_env->env[conidx]->cps.descs[CPPC_DESC_CTNL_PT_CL_CFG].desc_hdl != ATT_INVALID_SEARCH_HANDLE)
            {
                ...
                if (status == GAP_ERR_NO_ERROR)
                {
                    ...
                    prf_gatt_write(&(cppc_env->prf_env), conidx,
                                   cppc_env->env[conidx]->cps.chars[CPP_CPS_CTNL_PT_CHAR].val_hdl, (uint8_t *)&req[0],
                                   nb, GATTC_WRITE);
                }
            }
            else
    ...
    if (cppc_env != NULL)
    {
        ...
        if (state == CPPC_DISCOVERING)
        {
            ...
            if ((status == ATT_ERR_ATTRIBUTE_NOT_FOUND) || (status == ATT_ERR_NO_ERROR))
            {
                ...
                {
                    status =
                        prf_check_svc_char_validity(CPP_CPS_CHAR_MAX, cppc_env->env[conidx]->cps.chars, cppc_cps_char);
                }
                // too much services
                ...
                if (status == GAP_ERR_NO_ERROR)
                {
                    status = prf_check_svc_char_desc_validity(CPPC_DESC_MAX, cppc_env->env[conidx]->cps.descs,
                                  ...
                                  ke_task_id_t const src_id)
{
    ...
    if (state == CPPC_BUSY)
    {
        ...
        struct cppc_value_ind *ind =
            KE_MSG_ALLOC(CPPC_VALUE_IND, prf_dst_task_get(&(cppc_env->prf_env), conidx), dest_id, cppc_value_ind);

        switch (((struct cppc_read_cmd *)cppc_env->env[conidx]->operation)->read_code)
        {
        ...
        ind->att_code = ((struct cppc_read_cmd *)cppc_env->env[conidx]->operation)->read_code;
