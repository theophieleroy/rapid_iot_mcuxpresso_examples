...
const struct attm_desc hids_att_db[HOGPD_IDX_NB] = {
        ...
        [HOGPD_IDX_HID_CTNL_PT_VAL] = {ATT_CHAR_HID_CTNL_PT, PERM(WRITE_COMMAND, ENABLE), PERM(RI, ENABLE),
                                       sizeof(uint8_t)},
        ...
        [HOGPD_IDX_REPORT_MAP_VAL] = {ATT_CHAR_REPORT_MAP, PERM(RD, ENABLE), PERM(RI, ENABLE),
                                      HOGPD_REPORT_MAP_MAX_LEN},
        ...
        [HOGPD_IDX_PROTO_MODE_VAL] = {ATT_CHAR_PROTOCOL_MODE, (PERM(RD, ENABLE) | PERM(WRITE_COMMAND, ENABLE)),
        ...
        [HOGPD_IDX_BOOT_KB_IN_REPORT_VAL] = {ATT_CHAR_BOOT_KB_IN_REPORT, (PERM(RD, ENABLE) | PERM(NTF, ENABLE)),
                                             PERM(RI, ENABLE), HOGPD_BOOT_REPORT_MAX_LEN},
        // Boot Keyboard Input Report Characteristic - Client Characteristic Configuration Descriptor
        [HOGPD_IDX_BOOT_KB_IN_REPORT_NTF_CFG] = {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE),
                                                 0, 0},
        ...
        [HOGPD_IDX_BOOT_KB_OUT_REPORT_VAL] = {ATT_CHAR_BOOT_KB_OUT_REPORT, (PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE) |
        ...
        [HOGPD_IDX_BOOT_MOUSE_IN_REPORT_VAL] =
            {
                ATT_CHAR_BOOT_MOUSE_IN_REPORT, (PERM(RD, ENABLE) | PERM(NTF, ENABLE)), PERM(RI, ENABLE),
        ...
        [HOGPD_IDX_BOOT_MOUSE_IN_REPORT_NTF_CFG] =
            {ATT_DESC_CLIENT_CHAR_CFG, PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE), 0, 0},
        ...
        [HOGPD_IDX_REPORT_NTF_CFG] = {ATT_DESC_CLIENT_CHAR_CFG,
                                      PERM(RD, ENABLE) | PERM(WRITE_REQ, ENABLE) | PERM(WRITE_COMMAND, ENABLE), 0, 0},
};
 ...
 ****************************************************************************************
 */
static uint8_t hogpd_init(
    struct prf_task_env *env, uint16_t *start_hdl, uint16_t app_task, uint8_t sec_lvl, struct hogpd_db_cfg *params)
{
    ...
    for (svc_idx = 0; ((svc_idx < params->hids_nb) && (status == GAP_ERR_NO_ERROR)); svc_idx++)
    {
        ...
        hids_db[svc_idx] =
            (struct attm_desc *)ke_malloc(HOGPD_ATT_MAX * sizeof(struct attm_desc), KE_MEM_NON_RETENTION);
        ...
        for (report_idx = 0; report_idx < params->cfg[svc_idx].report_nb; report_idx++)
        {
            ...
            memcpy(&(hids_db[svc_idx][HOGPD_IDX_REPORT_CHAR + report_offset]), &(hids_att_db[HOGPD_IDX_REPORT_CHAR]),
    ...
    {
        uint16_t handle;
        status = attm_svc_create_db(&shdl, ATT_SVC_HID, (uint8_t *)&(cfg_flag[svc_idx][0]), HOGPD_ATT_MAX, NULL,
                                    env->task, hids_db[svc_idx],
        ...
        if (status == GAP_ERR_NO_ERROR)
        {
            ...
            status =
                attm_att_set_value(handle, sizeof(struct hids_hid_info), 0, (uint8_t *)&params->cfg[svc_idx].hid_info);
        }
            ...
            ((params->cfg[svc_idx].svc_features & HOGPD_CFG_MAP_EXT_REF) == HOGPD_CFG_MAP_EXT_REF))
        {
            ...
            ASSERT_ERR(handle != ATT_INVALID_HDL);
            status =
                attm_att_set_value(handle, sizeof(uint16_t), 0, (uint8_t *)&params->cfg[svc_idx].ext_ref.rep_ref_uuid);
        }

        // Set Report External Ref value
        for (report_idx = 0; (status == GAP_ERR_NO_ERROR) && (report_idx < params->cfg[svc_idx].report_nb);
             report_idx++)
        {
            // Save the Report ID
    ...
    if (status == ATT_ERR_NO_ERROR)
    {
        ...
        env->desc.idx_max = HOGPD_IDX_MAX;
        env->desc.state = hogpd_env->state;
        env->desc.default_handler = &hogpd_default_handler;
...
const struct prf_task_cbs hogpd_itf = {
    (prf_init_fnct)hogpd_init, hogpd_destroy, hogpd_create, hogpd_cleanup,
};
        ...
        ((att_idx < HOGPD_ATT_UNIQ_NB) || (report_idx < hogpd_env->svcs[svc_idx].nb_report)))
    {
        ...
        if (att_idx < HOGPD_ATT_UNIQ_NB)
        {
            ...
            if ((hogpd_env->svcs[svc_idx].features & HOGPD_CFG_KEYBOARD) == 0)
            {
                ...
                // Error Case
                else if ((att_idx >= HOGPD_IDX_BOOT_KB_IN_REPORT_CHAR) && (att_idx <= HOGPD_IDX_BOOT_KB_OUT_REPORT_VAL))
                {
            ...
            if ((hogpd_env->svcs[svc_idx].features & HOGPD_CFG_PROTO_MODE) == 0)
            {
                ...
                // Error Case
                else if ((att_idx >= HOGPD_IDX_PROTO_MODE_CHAR) && (att_idx <= HOGPD_IDX_PROTO_MODE_VAL))
                {
                    handle = ATT_INVALID_HANDLE;
                }
            }
...
uint8_t hogpd_get_att_idx(
    struct hogpd_env_tag *hogpd_env, uint16_t handle, uint8_t *svc_idx, uint8_t *att_idx, uint8_t *report_idx)
{
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        struct gattc_send_evt_cmd *report_ntf =
            KE_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD, KE_BUILD_ID(TASK_GATTC, conidx),
                             prf_src_task_get(&(hogpd_env->prf_env), conidx), gattc_send_evt_cmd, report->length);
...
uint8_t hogpd_ntf_cfg_ind_send(uint8_t conidx, uint8_t svc_idx, uint8_t att_idx, uint8_t report_idx, uint16_t ntf_cfg)
{
    ...
    {
        // Allocate the GATT notification message
        struct hogpd_ntf_cfg_ind *ntf_cfg_ind =
            KE_MSG_ALLOC(HOGPD_NTF_CFG_IND, prf_dst_task_get(&(hogpd_env->prf_env), conidx),
