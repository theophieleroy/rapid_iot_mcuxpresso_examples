    ...
    if (state == PASPS_IDLE)
    {
        ...
        if (status == GAP_ERR_NO_ERROR)
        {
            ...
            if (PASPS_IS_NTF_ENABLED(conidx, pasps_env, flag))
            {
                ...
                struct gattc_send_evt_cmd *ntf = KE_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD, KE_BUILD_ID(TASK_GATTC, conidx),
                                                                  dest_id, gattc_send_evt_cmd, sizeof(uint8_t));
                                          ...
                                          ke_task_id_t const src_id)
{
    if (ke_state_get(dest_id) == PASPS_IDLE)
    {
        ...
        if ((att_idx == PASS_IDX_ALERT_STATUS_CFG) || (att_idx == PASS_IDX_RINGER_SETTING_CFG))
        {
            ...
            (param->handle == (pasps_env->shdl + PASS_IDX_RINGER_SETTING_CFG)))
        {
            ...
            if ((ntf_cfg == PRF_CLI_STOP_NTFIND) || (ntf_cfg == PRF_CLI_START_NTF))
            {
                struct pasps_written_char_val_ind *ind =
                    KE_MSG_ALLOC(PASPS_WRITTEN_CHAR_VAL_IND, prf_dst_task_get(&pasps_env->prf_env, conidx),
                                 prf_src_task_get(&pasps_env->prf_env, conidx), pasps_written_char_val_ind);
                                 ...
                                 ke_task_id_t const src_id)
{
    ...
    if (pasps_env != NULL)
    {
        ...
        pasps_send_cmp_evt(prf_src_task_get(&pasps_env->prf_env, conidx), prf_dst_task_get(&pasps_env->prf_env, conidx),
