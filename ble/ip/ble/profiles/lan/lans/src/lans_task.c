...
static int lans_enable_req_handler(ke_msg_id_t const msgid,
        ...
        switch (att_idx)
        {
            ...
            case LNS_IDX_NAVIGATION_NTF_CFG:
            {
                ...
                if (LANS_IS_FEATURE_SUPPORTED(lans_env->prfl_cfg, LANS_NAVI_MASK))
                {
                    ...
                    co_write16p(cfm->value,
                                (lans_env->env[conidx].prfl_ntf_ind_cfg & LANS_PRF_CFG_FLAG_NAVIGATION_NTF) ?
                                    PRF_CLI_START_NTF :
                                    PRF_CLI_STOP_NTFIND);
                }
                else
            ...
            case LNS_IDX_LN_CTNL_PT_IND_CFG:
            {
                ...
                if (LANS_IS_FEATURE_SUPPORTED(lans_env->prfl_cfg, LANS_LN_CTNL_PT_MASK))
                {
                    ...
                    // Fill data
                    co_write16p(cfm->value,
                                (lans_env->env[conidx].prfl_ntf_ind_cfg & LANS_PRF_CFG_FLAG_LN_CTNL_PT_IND) ?
                                    PRF_CLI_START_IND :
                                    PRF_CLI_STOP_NTFIND);
                }
                else
                                          ...
                                          ke_task_id_t const src_id)
{
    if (ke_state_get(dest_id) == LANS_IDLE)
    {
        ...
        if ((att_idx == LNS_IDX_LOC_SPEED_NTF_CFG) || (att_idx == LNS_IDX_NAVIGATION_NTF_CFG) ||
 ...
 ****************************************************************************************
 */
static int lans_ntf_loc_speed_req_handler(ke_msg_id_t const msgid,
 ...
 ****************************************************************************************
 */
static int lans_upd_pos_q_req_handler(ke_msg_id_t const msgid,
                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    struct lans_upd_pos_q_rsp *rsp = KE_MSG_ALLOC(LANS_UPD_POS_Q_RSP, src_id, dest_id, lans_upd_pos_q_rsp);
 ...
 ****************************************************************************************
 */
static int lans_ntf_navigation_req_handler(ke_msg_id_t const msgid,
    ...
    if (ke_state_get(dest_id) == LANS_BUSY)
    {
        do
        {
            ...
            struct gattc_send_evt_cmd *ctl_pt_rsp =
                KE_MSG_ALLOC_DYN(GATTC_SEND_EVT_CMD, KE_BUILD_ID(TASK_GATTC, param->conidx), dest_id,
        ...
        if (status != GAP_ERR_NO_ERROR)
        {
            ...
            lans_send_cmp_evt(param->conidx, prf_src_task_get(&lans_env->prf_env, param->conidx),
                              prf_dst_task_get(&lans_env->prf_env, param->conidx), lans_env->operation, status);
        }
    }
                                       ...
                                       ke_task_id_t const src_id)
{
    ...
    if ((param->handle == (LANS_HANDLE(LNS_IDX_LN_CTNL_PT_VAL))) && (ctl_pt_status != LANP_LN_CTNL_PT_RESP_SUCCESS))
    {
        ...
        switch (param->operation)
        {
            ...
            case (GATTC_INDICATE):
            {
                ...
                lans_send_cmp_evt(conidx, prf_src_task_get(&(lans_env->prf_env), conidx),
                                  prf_dst_task_get(&(lans_env->prf_env), conidx), lans_env->operation, param->status);
                // else ignore the message
            }
            break;
