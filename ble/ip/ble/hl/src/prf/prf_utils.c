...
#if (BLE_CLIENT_PRF)
void prf_read_char_send(prf_env_t *prf_env, uint8_t conidx, uint16_t shdl, uint16_t ehdl, uint16_t valhdl)
{
...
void prf_register_atthdl2gatt(prf_env_t *prf_env, uint8_t conidx, struct prf_svc *svc)
{
    if (svc->shdl != ATT_INVALID_HANDLE)
    {
        ...
        struct gattc_reg_to_peer_evt_cmd *reg =
            KE_MSG_ALLOC(GATTC_REG_TO_PEER_EVT_CMD, KE_BUILD_ID(TASK_GATTC, conidx), prf_src_task_get(prf_env, conidx),
...
void prf_disc_svc_send(prf_env_t *prf_env, uint8_t conidx, uint16_t uuid)
{
    ...
    struct gattc_sdp_svc_disc_cmd *svc_req =
        KE_MSG_ALLOC_DYN(GATTC_SDP_SVC_DISC_CMD, KE_BUILD_ID(TASK_GATTC, conidx), prf_src_task_get(prf_env, conidx),
...
void prf_disc_svc_by_uuid_send(prf_env_t *prf_env, uint8_t conidx, uint8_t uuid_len, uint8_t *uuid)
{
    uint8_t uuid_length = (uuid_len == ATT_UUID_16_LEN) ?
                              ATT_UUID_16_LEN :
                              (uuid_len == ATT_UUID_32_LEN ? ATT_UUID_32_LEN : ATT_UUID_128_LEN);
    // send GATT discover primary services by UUID request
    struct gattc_sdp_svc_disc_cmd *svc_req =
        KE_MSG_ALLOC_DYN(GATTC_SDP_SVC_DISC_CMD, KE_BUILD_ID(TASK_GATTC, conidx), prf_src_task_get(prf_env, conidx),
                         gattc_sdp_svc_disc_cmd, uuid_length);
    ...
    // UUID search
    svc_req->uuid_len = uuid_length;

    // set the first two bytes to the value array, LSB to MSB:Health Thermometer Service UUID first
    memcpy(&(svc_req->uuid[0]), uuid, uuid_length);
...
void prf_gatt_write(
    prf_env_t *prf_env, uint8_t conidx, uint16_t handle, uint8_t *value, uint16_t length, uint8_t operation)
{
    if (handle != ATT_INVALID_HANDLE)
    {
        struct gattc_write_cmd *wr_char = KE_MSG_ALLOC_DYN(GATTC_WRITE_CMD, KE_BUILD_ID(TASK_GATTC, conidx),
...
uint8_t prf_check_svc_char_128_validity(uint8_t nb_chars,
                                        const struct prf_char_inf *chars,
                                        const struct prf_char_128_def *chars_req)
{
    uint8_t status = GAP_ERR_NO_ERROR;
    uint8_t i;
            ...
            if ((val_hdl <= param->end_hdl) && (val_hdl > (param->start_hdl + fnd_att)))
            {
                ...
                for (svc_char = 0; svc_char < nb_chars; svc_char++)
                {
                    ...
                    if ((chars[svc_char].char_hdl == ATT_INVALID_HDL) &&
                        attm_uuid16_comp((uint8_t *)param->info[val_idx].att.uuid, param->info[val_idx].att.uuid_len,
...
void prf_extract_svc_128_info(const struct gattc_sdp_svc_ind *param,
                              uint8_t nb_chars,
                              const struct prf_char_128_def *chars_req,
                              struct prf_char_inf *chars,
                              uint8_t nb_descs,
                ...
                for (svc_char = 0; svc_char < nb_chars; svc_char++)
                {
                    ...
                    if ((chars[svc_char].char_hdl == ATT_INVALID_HDL) &&
                        attm_uuid_comp((uint8_t *)param->info[val_idx].att.uuid, param->info[val_idx].att.uuid_len,
                                       chars_req[svc_char].uuid, chars_req[svc_char].uuid_len))
                    {
                        // Save properties and handles
                        chars[svc_char].char_hdl = char_hdl;
                        chars[svc_char].val_hdl = val_hdl;
...
void prf_extract_svc_uuid128_info(const struct gattc_sdp_svc_ind *param,
                                  uint8_t nb_chars,
                                  const struct prf_char_uuid128_def *chars_uuid128_req,
                                  struct prf_char_inf *chars,
                                  uint8_t nb_descs,
                                  const struct prf_char_desc_uuid128_def *descs_uuid128_req,
                                  struct prf_char_desc_inf *descs)
{
    // Counters
    uint8_t svc_char;
        ...
        if (param->info[fnd_att].att_type == GATTC_SDP_ATT_CHAR)
        {
            ...
            if ((val_hdl <= param->end_hdl) && (val_hdl > (param->start_hdl + fnd_att)))
            {
                ...
                for (svc_char = 0; svc_char < nb_chars; svc_char++)
                {
                    ...
                    if ((chars[svc_char].char_hdl == ATT_INVALID_HDL) &&
                        attm_uuid_comp((uint8_t *)param->info[val_idx].att.uuid, param->info[val_idx].att.uuid_len,
                                       &chars_uuid128_req[svc_char].uuid[0], chars_uuid128_req[svc_char].uuid_len))
                    {
                        // Save properties and handles
                        chars[svc_char].char_hdl = char_hdl;
                        chars[svc_char].val_hdl = val_hdl;
                            ...
                            if (param->info[fnd_att].att_type == GATTC_SDP_ATT_DESC)
                            {
                                ...
                                for (svc_desc = 0; svc_desc < nb_descs; svc_desc++)
                                {
                                    ...
                                    if ((descs[svc_desc].desc_hdl == ATT_INVALID_HANDLE) &&
                                        ...
                                        (attm_uuid_comp(
                                            (uint8_t *)param->info[fnd_att].att.uuid, param->info[fnd_att].att.uuid_len,
                                            descs_uuid128_req[svc_desc].uuid, descs_uuid128_req[svc_desc].uuid_len)))
                                    {
                                        descs[svc_desc].desc_hdl = param->start_hdl + 1 + fnd_att;
                                        // search for next descriptor
                                        break;
...
void prf_client_att_info_rsp(
    prf_env_t *prf_env, uint8_t conidx, uint16_t msg_id, uint8_t status, struct gattc_read_ind const *read_ind)
{
    ...
    // prepare response
    struct prf_att_info *rsp = KE_MSG_ALLOC_DYN(msg_id, prf_dst_task_get(prf_env, conidx),
