...
#if (BLE_QPP_SERVER)
extern const struct prf_task_cbs *qpps_prf_itf_get(void);
#endif //(BLE_QPP_SERVER)

#if (BLE_QPP_CLIENT)
extern const struct prf_task_cbs *qppc_prf_itf_get(void);
#endif //(BLE_QPP_CLIENT)

#if (BLE_OTA_CLIENT)
extern const struct prf_task_cbs *otac_prf_itf_get(void);
#endif //(BLE_OTA_CLIENT)
#if (BLE_OTA_SERVER)
extern const struct prf_task_cbs *otas_prf_itf_get(void);
#endif //(BLE_OTA_SERVER)
#if (BLE_YELL_SERVER)
extern const struct prf_task_cbs *yells_prf_itf_get(void);
...
#if (BLE_QPP_SERVER)
        case TASK_ID_QPPS:
            prf_cbs = qpps_prf_itf_get();
            break;
#endif //(BLE_QPP_SERVER)

#if (BLE_QPP_CLIENT)
        case TASK_ID_QPPC:
            prf_cbs = qppc_prf_itf_get();
            break;
#endif //(BLE_QPP_CLIENT)

#if (BLE_OTA_CLIENT)
        case TASK_ID_OTAC:
            prf_cbs = otac_prf_itf_get();
            break;
#endif //(BLE_OTA_CLIENT)
#if (BLE_OTA_SERVER)
        case TASK_ID_OTAS:
            prf_cbs = otas_prf_itf_get();
            break;
#endif //(BLE_OTA_SERVER)
#if (BLE_YELL_SERVER)
        case TASK_ID_YELLS:
            prf_cbs = yells_prf_itf_get();
            break;
#endif //(BLE_YELL_SERVER)
...
uint8_t prf_add_profile(struct gapm_profile_task_add_cmd *params, ke_task_id_t *prf_task)
{
    ...
    if (status == GAP_ERR_NO_ERROR)
    {
        ...
        for (i = 0; i < BLE_NB_PROFILES; i++)
        {
            ...
            {
                // initialize profile
                status = cbs->init(&(prf_env.prf[i]), &(params->start_hdl), params->app_task, params->sec_lvl,
                                   params->param);
