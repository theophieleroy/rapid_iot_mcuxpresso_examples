...
#define BLE_WHITELIST_MAX (ble_config.fw->ble_whitelist_max)
...
#define BLE_RESOL_ADDR_LIST_MAX (ble_config.fw->ble_resol_addr_list_max)

/// Connection handle max
#define BLE_MAX_CONHDL (FW_BLE_CONNECTION_MAX)

/// Number of devices capacity for the scan filtering
#if (BLE_CENTRAL || BLE_OBSERVER)
#define BLE_DUPLICATE_FILTER_MAX (ble_config.fw->ble_duplicate_filter_max)
...
#define BLE_HEAP_MSG_SIZE \
    (256 * (BLE_CONNECTION_MAX + 1) + 80 * (BLE_CONNECTION_MAX) + 96 * (2 * BLE_CONNECTION_MAX + 1))
/// Size required to allocate environment variable for one link
#define BLE_HEAP_ENV_SIZE (sizeof(struct llc_env_tag) + 4)
#define BLE_TAG_HEAP_ENV_SIZE (sizeof(struct llc_env_tag *) * BLE_CONNECTION_MAX + 4)
//#define BLE_TX_NODE_HEAP_ENV_SIZE       (sizeof(struct em_buf_node) * (10 * BLE_CONNECTION_MAX + 4))
//#define BLE_TX_NODE_HEAP_ENV_SIZE       (sizeof(struct em_buf_node) * (12 * BLE_CONNECTION_MAX + 25) + 8)
#define BLE_TX_NODE_HEAP_ENV_SIZE                                   \
    (sizeof(struct em_desc_node) * (11 * BLE_CONNECTION_MAX + 23) + \
     sizeof(struct em_buf_node) * (BLE_CONNECTION_MAX + 2) + 8)
#define BLE_KE_HEAP_ENV_SIZE (sizeof(struct ke_task_elem) * 16 + 4) // WJ: temp
#else
#define BLE_HEAP_MSG_SIZE (256)
/// Size required to allocate environment variable for one link
#define BLE_HEAP_ENV_SIZE (4)
#define BLE_TAG_HEAP_ENV_SIZE (4)
#define BLE_TX_NODE_HEAP_ENV_SIZE (sizeof(struct em_desc_node) * (23) + 8)
#define BLE_KE_HEAP_ENV_SIZE (sizeof(struct ke_task_elem) * 16 + 4)
...
#define RWBLE_PROG_LATENCY_DFT (ble_config.app->prog_latency_def)
/// EA asap latency
...
// byte10
#define BLE_CMDS_BYTE10 (BLE_HL_NB_CMP_PKT_CMD | BLE_RD_TX_PWR_CMD | BLE_HL_BUF_SIZE_CMD | BLE_SET_CTRL_TO_HL_FCTRL_CMD)
// byte14
#define BLE_CMDS_BYTE14 (BLE_RD_LOC_VERS_CMD | BLE_RD_LOC_SUP_FEAT_CMD | BLE_RD_BUF_SIZE_CMD)
// byte15
...
#endif
// byte25
#define BLE_CMDS_BYTE25                                                                                        \
    (BLE_LE_SET_EVT_MSK_CMD | BLE_LE_RD_BUF_SIZE_CMD | BLE_LE_RD_LOC_SUP_FEAT_CMD | BLE_LE_SET_RAND_ADDR_CMD | \
     BLE_LE_SET_ADV_PARAM_CMD | BLE_LE_RD_ADV_TX_PWR_CMD | BLE_LE_SET_ADV_DATA_CMD)
// byte26
#define BLE_CMDS_BYTE26                                                                                    \
    (BLE_LE_SET_SC_RSP_DATA_CMD | BLE_LE_SET_ADV_EN_CMD | BLE_LE_SET_SC_PARAM_CMD | BLE_LE_SET_SC_EN_CMD | \
     BLE_LE_CREAT_CNX_CMD | BLE_LE_CREAT_CNX_CNL_CMD | BLE_LE_RD_WL_SIZE_CMD | BLE_LE_CLEAR_WL_CMD)
// byte27
#define BLE_CMDS_BYTE27                                                                                   \
    (BLE_LE_ADD_DEV_WL_CMD | BLE_LE_REM_DEV_WL_CMD | BLE_LE_CNX_UPDATE_CMD | BLE_LE_SET_HL_CH_CLASS_CMD | \
     BLE_LE_RD_CH_MAP_CMD | BLE_LE_RD_REM_USED_FEAT_CMD | BLE_LE_ENCRYPT_CMD | BLE_LE_RAND_CMD)
// byte28
#define BLE_CMDS_BYTE28                                                                                         \
    (BLE_LE_START_ENC_CMD | BLE_LE_LTK_REQ_RPLY_CMD | BLE_LE_LTK_REQ_NEG_RPLY_CMD | BLE_LE_RD_SUPP_STATES_CMD | \
     BLE_LE_RX_TEST_CMD | BLE_LE_TX_TEST_CMD | BLE_LE_STOP_TEST_CMD)
#if !(BLE_QUALIF)
// byte32
#define BLE_CMDS_BYTE32 (BLE_RD_AUTH_PAYL_TO_CMD | BLE_WR_AUTH_PAYL_TO_CMD)
// byte33
#define BLE_CMDS_BYTE33                                                                                  \
    (BLE_LE_REM_CON_PARA_REQ_RPLY_CMD | BLE_LE_REM_CON_PARA_REQ_NEG_RPLY_CMD | BLE_LE_SET_DATA_LEN_CMD | \
     BLE_LE_RD_SUGGTED_DFT_DATA_LEN_CMD)
// byte34
#define BLE_CMDS_BYTE34                                                                                       \
    (BLE_LE_WR_SUGGTED_DFT_DATA_LEN_CMD | BLE_LE_RD_LOC_P256_PUB_KEY_CMD | BLE_LE_GEN_DH_KEY_CMD |            \
     BLE_LE_ADD_DEV_TO_RESOLV_LIST_CMD | BLE_LE_REM_DEV_FROM_RESOLV_LIST_CMD | BLE_LE_CLEAR_RESOLV_LIST_CMD | \
     BLE_LE_RD_RESOLV_LIST_SIZE_CMD | BLE_LE_RD_PEER_RESOLV_ADDR_CMD)
#if (BLE_2MBPS)
// byte35
#define BLE_CMDS_BYTE35                                                                                 \
    (BLE_LE_RD_LOCAL_RESOLV_ADDR_CMD | BLE_LE_SET_ADDR_RESOL_CMD | BLE_LE_SET_RESOLV_PRIV_ADDR_TO_CMD | \
     BLE_LE_RD_MAX_DATA_LEN_CMD | BLE_LE_RD_PHY_CMD | BLE_LE_SET_DFT_PHY_CMD | BLE_LE_SET_PHY_CMD |     \
     BLE_LE_ENH_RX_TEST_CMD)

// byte36
#define BLE_CMDS_BYTE36 (BLE_LE_ENH_RX_TEST_CMD)
...
#define BLE_NB_MAX_PACKET_PROG (32)
