...
static int32_t FLASH_GetResource(cmsis_flash_resource_t *resource, uint32_t address, uint32_t lengthInBytes)
{
...
#endif
    {
        ...
        if ((address >= DFlashBlockBase) &&
             ((address + lengthInBytes) <= (DFlashBlockBase + DFlashTotalSize)))
        {
        ...
        if ((address >= PFlashBlockBase) &&
             ((address + lengthInBytes) <= (PFlashBlockBase + PFlashTotalSize)))
        {
            resource->instance = flashIndex;
            if (resource->instance == 0)
            {
...
static void FLASH_UpdateInfo(flash_info_type_t infoType)
{
    switch (infoType)
    {
        ...
        case kFlashInfoType_Pflash1:
            s_flashInfo.sector_count = s_flashDriverState.config[1].PFlashTotalSize / s_flashDriverState.config[1].PFlashSectorSize;
        ...
        case kFlashInfoType_Dflash0:
            s_flashInfo.sector_count = s_flashDriverState.config[0].DFlashTotalSize / FSL_FEATURE_FLASH_FLEX_NVM_BLOCK_SECTOR_SIZE;
        ...
        default:
            s_flashInfo.sector_count = s_flashDriverState.config[0].PFlashTotalSize / s_flashDriverState.config[0].PFlashSectorSize;
...
static int32_t FLASH_ProgramData(uint32_t addr, const void *data, uint32_t cnt)
{
    ...
    if (result == ARM_DRIVER_OK)
    {
        ...
        status_t status = FLASH_Program(&s_flashDriverState.config[s_flashDriverState.resource.instance], addr, (uint32_t *)data, cnt);
        return FLASH_ConvertErrorStatus(status);
    }
...
static int32_t FLASH_EraseSector(uint32_t addr)
{
    ...
    if (result == ARM_DRIVER_OK)
    {
        ...
        status_t status = FLASH_Erase(&s_flashDriverState.config[s_flashDriverState.resource.instance],
                                      alignedAddress, s_flashDriverState.info->sector_size, kFLASH_ApiEraseKey);
        return FLASH_ConvertErrorStatus(status);
    }
